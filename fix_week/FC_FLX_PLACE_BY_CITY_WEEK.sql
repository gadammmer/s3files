------------------------------------------------------
---------------FC_FLX_PLACE_BY_CITY_WEEK--------------
------------------------------------------------------

-- ------AX_FLX_PLACE_BY_CITY_WEEK_PRE_ALL_TODAY----------------------------------------------------------------------------------------------------------------------------------


CREATE or REPLACE TABLE PROCESS.AX_FLX_PLACE_BY_CITY_WEEK_PRE_ALL_TODAY
(
...
WITH CONSOLIDADO AS (
    ...
    ,(select MAX(LOAD_DATE) LOAD_DATE from STAGING.ST_FLX_LUGARACTIVIDAD_DIAVISITA_MUNICIPIO_SEMANA) M
    where
    ...
    ,(select MAX(LOAD_DATE) LOAD_DATE from STAGING.ST_FLX_LUGARACTIVIDAD_VISPERA_MUNICIPIO_SEMANA) M
    ...
    ,(select MAX(LOAD_DATE) LOAD_DATE from STAGING.ST_FLX_LUGARNOCHE_DIAVISITA_MUNICIPIO_SEMANA) M
    where a.load_date=m.load_date
    AND (trim(CONTINUO) = 'nc' AND FILE_NAME NOT LIKE '%INVA-1080-6642%')
    AND FILE_NAME NOT LIKE '%INVA-1080-9999%'   
     union all
    ...
    ,(select MAX(LOAD_DATE) LOAD_DATE from STAGING.ST_FLX_LUGARNOCHE_VISPERA_MUNICIPIO_SEMANA) M
    where a.load_date=m.load_date
    AND (trim(CONTINUO) = 'nc' AND FILE_NAME NOT LIKE '%INVA-1080-6642%')
    AND FILE_NAME NOT LIKE '%INVA-1080-9999%'   
),
...
DECLARE
  ...
  COMPROBACION_INICIAL INT DEFAULT (SELECT COUNT(1) FROM (
                                        select count(1), WEEK_YEAR_KEY, ZONE_ID ,VISITOR_ORIGIN_ID ,VISITOR_CATEGORY_ID ,PLACE_PROVINCE_DES, PLACE_CITY_DES, CITY_DES ,KPI_ID                              
                                        from PROCESS.AX_FLX_PLACE_BY_CITY_WEEK_PRE_ALL_TODAY 
                                        where DUPLICATE_FILTER = 1
                                        group by WEEK_YEAR_KEY, ZONE_ID ,VISITOR_ORIGIN_ID ,VISITOR_CATEGORY_ID ,PLACE_PROVINCE_DES, PLACE_CITY_DES, CITY_DES ,KPI_ID
                                        having count(1) > 2
                                        ORDER BY WEEK_YEAR_KEY, ZONE_ID ,VISITOR_ORIGIN_ID ,VISITOR_CATEGORY_ID ,PLACE_PROVINCE_DES, PLACE_CITY_DES, CITY_DES ,KPI_ID));
  COMPROBACION_FINAL INT;

BEGIN
	IF (COMPROBACION_INICIAL > 0 ) THEN	
         RETURN 'Proceso cancelado: Hay duplicados en AX_FLX_PLACE_BY_CITY_WEEK_PRE_ALL_TODAY: ' || COMPROBACION_INICIAL;
    ELSE          
        
        -- ------AX_FLX_PLACE_BY_CITY_WEEK_ALL_TODAY-----------------------------------------
        
        
        
        
        CREATE or REPLACE TABLE PROCESS.AX_FLX_PLACE_BY_CITY_WEEK_ALL_TODAY
        (
          WEEK_YEAR_KEY NUMBER (10,0) ,
          ZONE_ID  VARCHAR(100) ,
          VISITOR_ORIGIN_ID  VARCHAR(20 CHAR) ,
        ...
        CREATE or REPLACE TABLE PROCESS.AX_FLX_PLACE_BY_CITY_WEEK
        (
          WEEK_YEAR_KEY NUMBER (10,0) ,
        ...
        CREATE or REPLACE TABLE SIT.FC_FLX_PLACE_BY_CITY_WEEK
        (
          WEEK_YEAR_KEY NUMBER (10,0) ,
          ZONE_KEY  NUMBER (16,0) ,
          VISITOR_ORIGIN_KEY  NUMBER (16,0) ,
...
----------------------------------------------------------
-- ------FC_FLX_PLACE_BY_CITY-----------------------------
----------------------------------------------------------

COMPROBACION_FINAL := (SELECT COUNT(1) FROM (
                                        select count(1), WEEK_YEAR_KEY, ZONE_KEY, VISITOR_ORIGIN_KEY, VISITOR_CATEGORY_KEY, CITY_KEY, PLACE_CITY_KEY, kpi_key  ,ZIPCODE_ID                                   
                                        from PROCESS.AX_FLX_PLACE_BY_CITY_WEEK 
                                        group by WEEK_YEAR_KEY, ZONE_KEY, VISITOR_ORIGIN_KEY, VISITOR_CATEGORY_KEY, CITY_KEY, PLACE_CITY_KEY, kpi_key,ZIPCODE_ID
                                        having count(1) > 1)
                                );                           

		IF (COMPROBACION_FINAL > 0 ) THEN	
            RETURN 'Proceso cancelado: Hay duplicados en AX_FLX_PLACE_BY_CITY_WEEK: ' || COMPROBACION_FINAL;
        ELSE
