...
CREATE or REPLACE TABLE PROCESS.AX_FLX_BY_AGE_MONTH_PRE_ALL_TODAY
(
...
,(select DISTINCT LOAD_DATE from STAGING.ST_FLX_DIARIO_MES_EDAD) M
...
,(select DISTINCT LOAD_DATE from STAGING.ST_FLX_NOCTURNO_MES_EDAD) M
where a.load_date=m.load_date  
AND (trim(CONTINUO) = 'nc' AND FILE_NAME NOT LIKE '%INVA-1080-6642%')
AND FILE_NAME NOT LIKE '%INVA-1080-9999%'   
),
...
CREATE or REPLACE TABLE PROCESS.AX_FLX_BY_AGE_MONTH_ALL_TODAY
(
YEAR_KEY NUMBER (10,0) ,
MONTH_KEY NUMBER (10,0) ,
ZONE_ID  VARCHAR(100) ,
...
CREATE or REPLACE TABLE PROCESS.AX_FLX_BY_AGE_MONTH
(
YEAR_KEY NUMBER (10,0) ,
MONTH_KEY NUMBER (10,0) ,
...
(select distinct YEAR_KEY,MONTH_KEY,zone_ID,kpi_id 
...
IF (COMPROBACION_FINAL > 0) THEN
    ...
    -- Mensaje del return
    RETURN 'Proceso cancelado: Hay duplicados en AX_FLX_BY_AGE_PRE_ALL_TODAY: ' || COMPROBACION_FINAL;	
ELSE

CREATE or REPLACE TABLE SIT.FC_FLX_BY_AGE_MONTH
(
YEAR_KEY NUMBER (10,0) ,
MONTH_KEY NUMBER (10,0) ,
ZONE_KEY  NUMBER (16,0) ,
...
CREATE or REPLACE TABLE PROCESS.AX_FLX_BY_AGE_PRE_ALL_TODAY
(
  PERIOD_KEY NUMBER (16,0) ,
  ZONE_ID  VARCHAR(100) ,
  VISITOR_ORIGIN_ID VARCHAR2(20 CHAR) ,
  VISITOR_CATEGORY_ID  VARCHAR(100) ,
  AGE_ID VARCHAR2(20 CHAR),
  KPI_ID VARCHAR(100) ,
  KPI_VALUE_NUM FLOAT,
  CONTINUO  VARCHAR(10) ,
  FECHA_CARGA DATE,
...
WITH CONSOLIDADO AS (
    ...
    ,(select DISTINCT LOAD_DATE from STAGING.ST_FLX_DIARIO_EDAD) M
	where a.load_date=m.load_date  
    AND (trim(CONTINUO) = 'nc' AND FILE_NAME NOT LIKE '%INVA-1080-6642%')
    AND FILE_NAME NOT LIKE '%INVA-1080-9999%'   
    ...
    ,(select DISTINCT LOAD_DATE from STAGING.ST_FLX_LLEGADA_EDAD) M
	where a.load_date=m.load_date  
    AND (trim(CONTINUO) = 'nc' AND FILE_NAME NOT LIKE '%INVA-1080-6642%')
    AND FILE_NAME NOT LIKE '%INVA-1080-9999%'   
    ...
    ,(select DISTINCT LOAD_DATE from STAGING.ST_FLX_SALIDA_EDAD) M
	where a.load_date=m.load_date  
    AND (trim(CONTINUO) = 'nc' AND FILE_NAME NOT LIKE '%INVA-1080-6642%')
    AND FILE_NAME NOT LIKE '%INVA-1080-9999%'   
    ...
    ,(select DISTINCT LOAD_DATE from STAGING.ST_FLX_NOCTURNO_EDAD) M
	where a.load_date=m.load_date  
    AND (trim(CONTINUO) = 'nc' AND FILE_NAME NOT LIKE '%INVA-1080-6642%')
    AND FILE_NAME NOT LIKE '%INVA-1080-9999%'   
),
...
BEGIN
  ...
  */      CREATE or REPLACE TABLE PROCESS.AX_FLX_BY_AGE_ALL_TODAY
        (
          PERIOD_KEY       NUMBER (16,0) ,
          ZONE_ID  VARCHAR(100) ,
          VISITOR_ORIGIN_ID  VARCHAR2(20 CHAR) ,
