...
(select DISTINCT LOAD_DATE from STAGING.ST_FLX_DURACIONESTANCIA) M	
...
WITH CONSOLIDADO AS (
    ...
    ,(select DISTINCT LOAD_DATE from STAGING.ST_FLX_DIARIO_MES) M
	where a.load_date=m.load_date  
    AND (trim(CONTINUO) = 'nc' AND FILE_NAME NOT LIKE '%INVA-1080-6642%')
    AND FILE_NAME NOT LIKE '%INVA-1080-9999%'   
    ...
    ,(select DISTINCT LOAD_DATE from STAGING.ST_FLX_NOCTURNO_MES) M
	where a.load_date=m.load_date  
    AND (trim(CONTINUO) = 'nc' AND FILE_NAME NOT LIKE '%INVA-1080-6642%')
    AND FILE_NAME NOT LIKE '%INVA-1080-9999%'   
),
