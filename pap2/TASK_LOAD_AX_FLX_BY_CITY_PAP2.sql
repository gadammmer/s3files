EXECUTE IMMEDIATE '
  BEGIN
        create or replace task PROCESS.LOAD_AX_FLX_BY_CITY
          ...
          as BEGIN
              ...
              WITH CONSOLIDADO AS (
                  ...
                  ,(select MAX(LOAD_DATE) LOAD_DATE from STAGING.ST_FLX_DIARIO_MES_MUNICIPIO) M
                where a.load_date=m.load_date  
                  ...
                  ,(select MAX(LOAD_DATE) LOAD_DATE from STAGING.ST_FLX_NOCTURNO_MES_MUNICIPIO) M
                where a.load_date=m.load_date  
                  AND (trim(CONTINUO) = ''nc'' AND FILE_NAME NOT LIKE ''%INVA-1080-6642%'')
                  AND FILE_NAME NOT LIKE ''%INVA-1080-9999%''   
              ),
