...
*/
    ...
    INSERT INTO PROCESS.WK_FLX_ALARM_DUPLICATE_LOG (DUPLICATE_DETAIL, DUPLICATE_COUNT, LOAD_TIME)
    SELECT 'Hay duplicados en AX_FLX_PLACE_BY_CITY_WEEK' AS DUPLICATE_DETAIL, COUNT(1),  CURRENT_TIMESTAMP() FROM (
    select count(1), CITY_KEY, ZONE_KEY, VISITOR_ORIGIN_KEY, VISITOR_CATEGORY_KEY , KPI_KEY
    from PROCESS.AX_FLX_PLACE_BY_CITY_WEEK
    group by CITY_KEY, ZONE_KEY, VISITOR_ORIGIN_KEY, VISITOR_CATEGORY_KEY , KPI_KEY
    having count(1) > 1);
...
END;   
*/
CREATE or REPLACE TABLE PROCESS.AX_FLX_PLACE_BY_CITY_PRE_ALL_TODAY
(
...
WITH CONSOLIDADO AS (
    ...
    ,(select DISTINCT LOAD_DATE from DEV_TOURISM_DB.STAGING.ST_FLX_LUGARACTIVIDAD_DIAVISITA_MUNICIPIO) M
    where
    a.load_date=m.load_date
    ...
    ,(select DISTINCT LOAD_DATE from DEV_TOURISM_DB.STAGING.ST_FLX_LUGARACTIVIDAD_VISPERA_MUNICIPIO) M
    where
    a.load_date=m.load_date
    AND (trim(A.CONTINUO) = 'nc' AND FILE_NAME NOT LIKE '%INVA-1080-6642%')
    AND FILE_NAME NOT LIKE '%INVA-1080-9999%'
    ...
    ,(select DISTINCT LOAD_DATE from DEV_TOURISM_DB.STAGING.ST_FLX_LUGARNOCHE_DIAVISITA_MUNICIPIO) M
    where
    a.load_date=m.load_date
    AND (trim(A.CONTINUO) = 'nc' AND FILE_NAME NOT LIKE '%INVA-1080-6642%')
    AND FILE_NAME NOT LIKE '%INVA-1080-9999%'
    ...
    ,(select DISTINCT LOAD_DATE from DEV_TOURISM_DB.STAGING.ST_FLX_LUGARNOCHE_VISPERA_MUNICIPIO) M
    where
    a.load_date=m.load_date
    AND (trim(A.CONTINUO) = 'nc' AND FILE_NAME NOT LIKE '%INVA-1080-6642%')
    AND FILE_NAME NOT LIKE '%INVA-1080-9999%'
...
ELSE
*/
            CREATE or REPLACE TABLE PROCESS.AX_FLX_PLACE_BY_CITY_ALL_TODAY
        (
          PERIOD_KEY NUMBER (16,0) ,
          ZONE_ID  VARCHAR(100) ,
          VISITOR_ORIGIN_ID  VARCHAR(20 CHAR) ,
        ...
        CREATE or REPLACE TABLE PROCESS.AX_FLX_PLACE_BY_CITY
        (
          PERIOD_KEY NUMBER (16,0) ,
          ZONE_KEY  NUMBER (16,0) ,
          VISITOR_ORIGIN_KEY  NUMBER (16,0) ,
...
/*
        ...
        from PROCESS.AX_FLX_PLACE_BY_CITY 
        ...
        INSERT INTO PROCESS.WK_FLX_ALARM_DUPLICATE_LOG (DUPLICATE_DETAIL, DUPLICATE_COUNT, LOAD_TIME)
        SELECT 'Hay duplicados en AX_FLX_PLACE_BY_CITY' AS DUPLICATE_DETAIL, COUNT(1),  CURRENT_TIMESTAMP() FROM (
        select count(1), PERIOD_KEY, ZONE_KEY, VISITOR_ORIGIN_KEY, VISITOR_CATEGORY_KEY , KPI_KEY
        from PROCESS.AX_FLX_PLACE_BY_CITY
        group by PERIOD_KEY, ZONE_KEY, VISITOR_ORIGIN_KEY, VISITOR_CATEGORY_KEY , KPI_KEY
        having count(1) > 1);
        ...
        -- Mensaje del return
        RETURN 'Proceso cancelado: Hay duplicados en AX_FLX_PLACE_BY_CITY: ' || COMPROBACION_FINAL;	
        ELSE
    */
            CREATE or REPLACE TABLE SIT.FC_FLX_PLACE_BY_CITY
            (
            PERIOD_KEY  NUMBER (16,0) ,
            ZONE_KEY  NUMBER (16,0) ,
            VISITOR_ORIGIN_KEY  NUMBER (16,0) ,
...
/*
    ...
    INSERT INTO PROCESS.WK_FLX_ALARM_DUPLICATE_LOG (DUPLICATE_DETAIL, DUPLICATE_COUNT, LOAD_TIME)
    SELECT 'Hay duplicados en AX_FLX_PLACE_BY_COUNTRY_WEEK' AS DUPLICATE_DETAIL, COUNT(1),  CURRENT_TIMESTAMP() FROM (
    select count(1), PLACE_CITY_KEY, ZONE_KEY, VISITOR_ORIGIN_KEY, VISITOR_CATEGORY_KEY , KPI_KEY
    from PROCESS.AX_FLX_PLACE_BY_COUNTRY_WEEK
    group by PLACE_CITY_KEY, ZONE_KEY, VISITOR_ORIGIN_KEY, VISITOR_CATEGORY_KEY , KPI_KEY
    having count(1) > 1);
